...
class Admin extends CI_Controller {
	...
	function load_dashboard()
	{
		...
		// $sesndataa = $this->session->all_userdata();
		// print_r(json_encode($sesndataa['alluserdata']['userid']) );
		// exit();
		...
		if($this->session->userdata('userid') || ($this->input->cookie('loggedin') && $this->input->cookie('loggedin') == 'true' && empty($data['x'])))
    	{
			...
			{
	    $url = $this->user1->get_dashboard().$addn;
			$data['url'] = $url;
			$data['redirect'] = true;
			#redirect($url);
...
/*
 ...
 // print_r($str); exit();
	...
	function forgotpassword()
	{
		...
		$this->email->from('noreply@gpp.ppda.go.ug', 'Government of Uganda Procurement Portal  | Password Reset ');
		$this->email->to(''.$email.'');
